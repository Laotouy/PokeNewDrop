...
public class JsonToNBT
{
    ...
    protected NBTTagCompound readStruct() throws NBTException
    {
        ...
        while (this.canRead() && this.peek() != '}')
        {
            ...
            this.expect(':');
            nbttagcompound.set(s, this.readValue());
    ...
    private NBTBase readListTag() throws NBTException
    {
        ...
        else
        {
            ...
            while (this.peek() != ']')
            {
                ...
                {
                    throw this.exception("Unable to insert " + NBTBase.j(j) + " into ListTag of type " + NBTBase.j(i));
                }
    ...
    private <T extends Number> List<T> readArray(byte p_193603_1_, byte p_193603_2_) throws NBTException
    {
        ...
        while (true)
        {
            ...
            {
                NBTBase nbtbase = this.readValue();
                int i = nbtbase.getTypeId();

                if (i != p_193603_2_)
                {
                    throw this.exception("Unable to insert " + NBTBase.j(i) + " into " + NBTBase.j(p_193603_1_));
                }
